...
export default async function Home({ searchParams }: { searchParams: Promise<{ property?: string; start?: string; end?: string; warning?: string }> }) {
  ...
  return <Shell><div className="page-head"><div><h1>Shadow recommendations</h1><div className="muted">Canonical BookingSite prices. Nothing on Hostex is modified.</div></div><span className="pill">Shadow only</span></div>
    ...
    <div className="card calendar">{nights.length ? <table><thead><tr><th>Date</th><th>Property</th><th>Hostex current</th><th>Recommended</th><th>Difference</th><th>Warnings</th><th>Explanation</th></tr></thead><tbody>{nights.map(n => <tr key={`${n.property_id}-${n.date}`}><td>{n.date}</td><td>{n.property_name}</td><td>{n.actual_price == null ? "—" : idr.format(n.actual_price)}</td><td className="price">{idr.format(n.recommended_price)}</td><td className={(n.difference ?? 0) > 0 ? "positive" : (n.difference ?? 0) < 0 ? "negative" : ""}>{n.difference == null ? "—" : `${n.difference >= 0 ? "+" : ""}${idr.format(n.difference)} (${pct(n.difference_percentage)})`}</td><td>{n.warnings.length ? <div className="badges">{n.warnings.map(w => <span className="warning" key={w}>{warningLabel[w] ?? w}</span>)}</div> : <span className="muted">None</span>}</td><td><details><summary>Why this price</summary><div className="explanation-grid"><span>Price source</span><b>{String(n.explanation.price_source ?? "—")}</b><span>Airbnb guest median</span><b>{n.explanation.airbnb_guest_market_median == null ? "—" : idr.format(Number(n.explanation.airbnb_guest_market_median))}</b><span>Available / required competitors</span><b>{String(n.explanation.available_competitor_count ?? 0)} / {String(n.explanation.minimum_competitor_count ?? "—")}</b><span>Guest-to-host factor</span><b>{String(n.explanation.guest_to_host_price_factor ?? "—")}</b><span>Estimated host median</span><b>{n.explanation.estimated_host_price_median == null ? "—" : idr.format(Number(n.explanation.estimated_host_price_median))}</b><span>Positioning factor</span><b>{String(n.explanation.market_positioning_factor ?? "—")}</b><span>Base price</span><b>{idr.format(Number(n.explanation.base_price ?? 0))}</b><span>Days until stay</span><b>{String(n.explanation.days_until_stay ?? "—")}</b><span>Urgency adjustment</span><b>{n.explanation.urgency_adjustment_applied ? pct(Number(n.explanation.urgency_adjustment ?? 0)) : n.explanation.urgency_adjustment_enabled ? "Not applied (fallback/override)" : "Disabled"}</b><span>Raw / rounded / final</span><b>{idr.format(Number(n.explanation.raw_price ?? 0))} / {idr.format(Number(n.explanation.rounded_price ?? 0))} / {idr.format(Number(n.explanation.final_price ?? 0))}</b><span>Date override</span><b>{n.explanation.manual_override == null ? "No" : idr.format(Number(n.explanation.manual_override))}</b></div></details></td></tr>)}</tbody></table> : <div className="empty">No recommendations in this range. Market mode uses current or saved medians; manual mode uses the configured base price.</div>}</div>
