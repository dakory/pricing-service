...
export default async function Home({ searchParams }: { searchParams: Promise<{ property?: string; start?: string; end?: string; warning?: string }> }) {
  ...
  return <Shell><div className="page-head"><div><h1>Shadow recommendations</h1><div className="muted">Canonical BookingSite prices. Nothing on Hostex is modified.</div></div><span className="pill">Shadow only</span></div>
    ...
    <div className="card calendar">{nights.length ? <table><thead><tr><th>Date</th><th>Property</th><th>Hostex current</th><th>Recommended</th><th>Difference</th><th>Warnings</th><th>Explanation</th></tr></thead><tbody>{nights.map(n => <tr key={`${n.property_id}-${n.date}`}><td>{n.date}</td><td>{n.property_name}</td><td>{n.actual_price == null ? "—" : idr.format(n.actual_price)}</td><td className="price">{idr.format(n.recommended_price)}</td><td className={(n.difference ?? 0) > 0 ? "positive" : (n.difference ?? 0) < 0 ? "negative" : ""}>{n.difference == null ? "—" : `${n.difference >= 0 ? "+" : ""}${idr.format(n.difference)} (${pct(n.difference_percentage)})`}</td><td>{n.warnings.length ? <div className="badges">{n.warnings.map(w => <span className="warning" key={w}>{warningLabel[w] ?? w}</span>)}</div> : <span className="muted">None</span>}</td><td><details><summary>Why this price</summary><div className="explanation-grid"><span>Market median</span><b>{idr.format(Number(n.explanation.market_price ?? 0))}</b><span>Available competitors</span><b>{String(n.explanation.available_competitor_count ?? 0)}</b><span>Competitor unavailability</span><b>{pct(Number(n.explanation.competitor_unavailability ?? 0))}</b><span>Portfolio occupancy</span><b>{pct(Number(n.explanation.portfolio_occupancy ?? 0))}</b><span>Demand adjustment</span><b>{pct(Number(n.explanation.demand_adjustment ?? 0))}</b><span>Days until stay</span><b>{String(n.explanation.days_until_stay ?? "—")}</b><span>Urgency adjustment</span><b>{pct(Number(n.explanation.urgency_adjustment ?? 0))}</b><span>Raw / bounded</span><b>{idr.format(Number(n.explanation.raw_price ?? 0))} / {idr.format(Number(n.explanation.bounded_price ?? 0))}</b><span>Manual override</span><b>{n.explanation.manual_override == null ? "No" : idr.format(Number(n.explanation.manual_override))}</b></div></details></td></tr>)}</tbody></table> : <div className="empty">No v2 recommendations in this range. Competitor observations are required.</div>}</div>
