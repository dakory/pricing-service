...
export default function CalendarPage() {
  ...
  function selectDate(propertyId:number, date:string) {
    const current = selection;
    if (!current || current.propertyId !== propertyId || date < current.start) {
      setSelection({ propertyId, start:date, end:date });
    } else {
      setSelection({ ...current, end:date });
    }
  }
  async function saveSelection() {
    if (!selection || !editPrice || !editReason) return;
    const endpoint = editMode === "override" ? "/api/overrides" : "/api/price-anchors";
    const response = await fetch(endpoint, { method:"POST", headers:{"Content-Type":"application/json", "X-CSRF-Token":csrf()}, body:JSON.stringify({ property_id:selection.propertyId, start_date:selection.start, end_date:selection.end, price:Number(editPrice), reason:editReason }) });
    const body = await response.json().catch(() => ({}));
    setMessage(response.ok ? "Calendar change saved" : body.detail ?? "Could not save calendar change");
    if (response.ok) { setSelection(null); setEditPrice(""); setEditReason(""); await load(); }
  }
  ...
  return <Shell><div className="calendar-page">
    ...
    <div className="calendar-frame card"><div className="calendar-scroll"><table className="pricing-grid"><thead><tr><th className="property-sticky">Property</th>{dates.map(date => <th key={date}>{shortDate(date)}</th>)}</tr></thead><tbody>{Array.from(groups.entries()).map(([groupId, properties]) => <>{<tr className="group-row" key={`group-${groupId}`}><th className="property-sticky" colSpan={dates.length+1}>Pricing group {groupId}</th></tr>}{properties.map(property => <tr key={property.id}><th className="property-sticky property-name" onClick={() => setSelectedProperty(property)}><span className="property-dot" />{property.name}</th>{dates.map(date => { const day=byKey.get(`${property.id}:${date}`); const selected=selection?.propertyId===property.id && date>=selection.start && date<=selection.end; return <td key={date} onClick={() => day?.available && selectDate(property.id,date)} className={`${day?.available ? "" : "blocked"} ${day?.override ? "has-override" : ""} ${selected ? "selected-cell" : ""}`} title={day?.explanation ? JSON.stringify(day.explanation) : undefined}>{day?.available ? <><strong>{money(day.current_price)}</strong>{day.recommended_price != null && <small className={day.difference && day.difference < 0 ? "negative" : "positive"}>{money(day.recommended_price)} {day.difference_percentage == null ? "" : `${day.difference_percentage>=0?"+":""}${(day.difference_percentage*100).toFixed(1)}%`}</small>}</> : <span className="unavailable">Unavailable</span>}</td>; })}</tr>)}</>)}</tbody></table>{loading && <div className="calendar-loading">Loading calendar…</div>}{!loading && !data.properties.length && <div className="empty">No active properties are available.</div>}</div></div>
    {selection && <div className="selection-toolbar card"><span><b>{selection.start}</b> — <b>{selection.end}</b></span><select value={editMode} onChange={event => setEditMode(event.target.value as "override"|"anchor")}><option value="override">Set price override</option><option value="anchor">Set manual base anchor</option></select><input value={editPrice} onChange={event => setEditPrice(event.target.value)} type="number" min="1" placeholder="Price in IDR"/><input value={editReason} onChange={event => setEditReason(event.target.value)} placeholder="Reason"/><button className="button" onClick={() => void saveSelection()}>Save</button><button className="button secondary" onClick={() => setSelection(null)}>Cancel</button></div>}
