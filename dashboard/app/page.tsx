...
function InputWithSelectField({ label, value, options, placeholder="Enter a value" }: { label:string; value?:string; options:{label:string;value:string;linked?:boolean}[]; placeholder?:string }) {
  const [open,setOpen]=useState(false); const [current,setCurrent]=useState(value??"");
  return <label className="prototype-linked-field">{label}<div className={`prototype-linked-control ${open ? "focused" : ""}`}><button type="button" onClick={()=>setOpen(!open)}>{current || options.find(option=>option.value===current)?.label || placeholder}</button>{open && <div className="prototype-linked-menu">{options.map(option => <button type="button" key={option.value} className={option.value===current ? "selected" : ""} onClick={()=>{setCurrent(option.value);setOpen(false);}}>{option.linked ? <span className="linked-value">{option.label}</span> : option.label}</button>)}<input autoFocus value={current} onChange={event=>setCurrent(event.target.value)} placeholder={placeholder} /></div>}</div></label>;
}
...
export default function CalendarPage() {
  ...
  return <Shell><main className="prototype-dashboard">
    ...
    <Drawer open={drawer==="group"} onClose={() => setDrawer(null)} width={400}><div className="prototype-drawer-kicker">Pricing group</div><h2>Group {selectedGroup}</h2><p className="prototype-muted">Properties in this group inherit these settings.</p><div className="prototype-chip-row">{groups.find(group=>group.id===selectedGroup)?.properties.map(property => <span key={property.id}>{property.name}</span>)}</div><InputWithSelectField label="Minimum competitor count" value="" options={[{label:"Global: 10",value:"",linked:true}]} /><label>Competitor URLs<textarea rows={8} placeholder="One canonical Airbnb URL per line" /></label><button className="prototype-button accent full" onClick={() => {setDrawer(null);setNoticeFor("Pricing group saved");}}>Save</button></Drawer>
    <Drawer open={drawer==="property"} onClose={() => setDrawer(null)}><div className="prototype-drawer-kicker">Property settings</div><h2>{selectedProperty?.name}</h2><p className="prototype-muted">{selectedProperty?.booking_site_listing_id ? `BookingSite listing ${selectedProperty.booking_site_listing_id}` : "BookingSite listing not mapped"}</p><label className="prototype-switch"><input type="checkbox" defaultChecked /> Suggest pricing</label><h3>Base price</h3><div className="prototype-segmented"><button className="active">Market median</button><button>Manual</button></div><label>Market positioning factor<input defaultValue="1.0" /></label><InputWithSelectField label="Minimum competitor count" value="" options={[{label:"Global: 10",value:"",linked:true}]} /><h3>Discounts by days left until stay</h3><label className="prototype-switch"><input type="checkbox" defaultChecked /> Enable urgency adjustments</label><div className="prototype-urgency-chart"><i /><i /><i /><i /></div><h3>Bounds and rounding</h3><InputWithSelectField label="Minimum price" options={[{label:"Global minimum",value:"",linked:true}]} /><InputWithSelectField label="Maximum price" options={[{label:"Global maximum",value:"",linked:true}]} /><InputWithSelectField label="Round to nearest" options={[{label:"Global pricing step",value:"",linked:true}]} /><button className="prototype-button accent full" onClick={() => {setDrawer(null);setNoticeFor("Property settings saved");}}>Save</button></Drawer>
  </main></Shell>;
}
