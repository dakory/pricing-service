...
export function OverrideForm({ properties, selectedId, start, end }: { properties: Property[]; selectedId?: number; start: string; end: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = response.status === 204 ? {} : await response.json(); setMessage(response.ok ? "Override saved. Generate recommendations to apply it." : body.detail); router.refresh();
  }
...
export function ManualBaseForm({ properties, selectedId, start, end }: { properties: Property[]; selectedId?: number; start: string; end: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = await response.json(); setMessage(response.ok ? `${body.count} manual base anchor(s) saved. Generate recommendations to apply it.` : body.detail); router.refresh();
  }
