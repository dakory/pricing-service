...
type Override = { id: number; property_id: number; start_date: string; end_date: string; price: number | null; minimum_stay: number | null; reason: string };
...
export function OverrideList({ overrides, properties }: { overrides: Override[]; properties: Property[] }) {
  ...
  async function remove(id: number) { const response = await fetch(`/api/overrides/${id}`, { method: "DELETE", headers: { "X-CSRF-Token": csrf() } }); setMessage(response.ok ? "Override removed. Run shadow pricing to recalculate." : "Could not remove override."); router.refresh(); }
  if (!overrides.length) return null;
  return <div className="card override-list"><b>Active hard overrides</b>{overrides.map(item => <div className="override-row" key={item.id}><div><strong>{names[item.property_id]}</strong><span>{item.start_date} → {item.end_date}</span><small>{item.price ? idr.format(item.price) : "Price unchanged"} · {item.minimum_stay ? `${item.minimum_stay} nights` : "Stay unchanged"} · {item.reason}</small></div><button className="button danger" onClick={() => remove(item.id)}>Remove</button></div>)}{message && <p className="muted">{message}</p>}</div>;
}
