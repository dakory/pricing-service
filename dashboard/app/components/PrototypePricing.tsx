...
{months.map((m,idx)=>(
<div key={m.label} style={{gridColumn:`${m.start+2} / ${idx+1<months.length?months[idx+1].start+2:days.length+2}`,gridRow:1,position:'sticky',left:labelColWidth,top:0,height:48,boxSizing:'border-box',zIndex:2,background:'var(--color-white)',padding:'14px 16px',fontFamily:'var(--font-display)',fontWeight:600,fontSize:15,width:180,whiteSpace:'nowrap',overflow:'hidden',justifySelf:'start'}}>{m.label}</div>
))}
