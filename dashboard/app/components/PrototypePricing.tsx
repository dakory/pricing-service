// @ts-nocheck
"use client";
// The prototype is intentionally kept as a close JSX port; runtime data adapters are added around it.
import React from "react";
import ReactDOM from "react-dom";
const Input=({label, value, onChange, placeholder, prefix})=><label className="prototype-input">{label}<div>{prefix&&<span>{prefix}</span>}<input value={value??""} onChange={onChange} placeholder={placeholder}/></div></label>;
const IconButton=({label,onClick,icon})=><button aria-label={label} className="prototype-icon-button" onClick={onClick}>{icon||"×"}</button>;
const Select=({value,onChange,options})=><select value={value} onChange={onChange}>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>;
const Button=({children,onClick,disabled,className="",variant})=><button className={"prototype-button "+(variant||"")+" "+className} onClick={onClick} disabled={disabled}>{children}</button>;
const Toast=({children,onClose})=><div className="prototype-toast">{children}<button onClick={onClose}>×</button></div>;
const Switch=({checked,onChange,label})=><label className="prototype-switch">{label}<input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)}/></label>;
const InputWithSelectField=Input;
