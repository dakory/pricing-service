...
const [scrapeForm,setScrapeForm]=React.useState({groupId:'',start:new Date().toISOString().slice(0,10),end:new Date(Date.now()+6*86400000).toISOString().slice(0,10),force:false,mode:'precise'});
...
React.useEffect(()=>{fetch('/api/competitors',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(rows=>{setCompetitors(rows||[]);}).catch(()=>{});},[]);
React.useEffect(()=>{if(!scrapeForm.groupId&&pricingGroups.length)setScrapeForm(f=>({...f,groupId:String(pricingGroups[0].id)}));},[pricingGroups,scrapeForm.groupId]);
...
React.useEffect(()=>{if(!activeProperty?.id){setPropertySettingsLoading(false);return;}setPropertySettingsLoading(true);fetch('/api/settings/pricing/effective/'+activeProperty.id).then(r=>{if(!r.ok)throw new Error('Property settings request failed');return r.json();}).then(data=>{if(!data)throw new Error('Property settings are empty');const entry=key=>data[key]||{value:null,source:'default'};const value=key=>effectiveSetting(entry(key));const source=key=>entry(key).source;setPropertyData(d=>({...d,[activeProperty.name]:{...propFor(activeProperty.name),sources:Object.fromEntries(Object.keys(data).map(key=>[key,source(key)])),enabled:value('suggest_prices')!==false,method:value('base_price_mode')==='manual'?'manual':'median',manualBase:value('manual_base_price')?String(value('manual_base_price')):'',positioning:String(value('market_positioning_factor')),minComp:source('minimum_competitor_count')==='property'?String(value('minimum_competitor_count')||''):'',useUrgency:value('urgency_adjustment_enabled')?'on':'off',rules:normalizeUrgencyRules(value('urgency_adjustments')).map(r=>({f:r.minimum_days,t:r.maximum_days,d:r.adjustment})),minIDR:source('minimum_price')==='property'?String(value('minimum_price')||''):'',maxIDR:source('maximum_price')==='property'?String(value('maximum_price')||''):'',step:source('rounding_increment')==='property'?String(value('rounding_increment')||''):''}}));}).catch(error=>setDataError(error.message||'Could not load property settings.')).finally(()=>setPropertySettingsLoading(false));},[activeProperty?.id]);
const setPropField=(name,field,val)=>setPropertyData(d=>{const current=propFor(name);const sources={...(current.sources||{})};sources[field]=val===''?'inherit':'property';return {...d,[name]:{...current,[field]:val,sources}};});
const setPropRules=(name,updater)=>setPropertyData(d=>{const cur=propFor(name);const rules=typeof updater==='function'?updater(cur.rules):updater;return {...d,[name]:{...cur,rules,sources:{...(cur.sources||{}),urgency_adjustments:'property'}}};});
const saveProperty=(name)=>{
 ...
 const settings={suggest_prices:pd.enabled,base_price_mode:pd.method==='median'?'market_median':'manual',manual_base_price:pd.manualBase?Number(String(pd.manualBase).replace(/,/g,'')):null,market_positioning_factor:Number(pd.positioning),minimum_competitor_count:pd.minComp?Number(pd.minComp):null,urgency_adjustment_enabled:pd.useUrgency==='on',urgency_adjustments:pd.rules.map(r=>({minimum_days:r.f,maximum_days:r.t,adjustment:Number(r.d)}))};
 const settingKeys=['suggest_prices','base_price_mode','manual_base_price','market_positioning_factor','minimum_competitor_count','urgency_adjustment_enabled','urgency_adjustments'];
 const pricing_settings=Object.fromEntries(settingKeys.filter(key=>pd.sources?.[key]==='property'||pd.sources?.[key]==='inherit').map(key=>[key,settings[key]]));
 const payload={pricing_group_id:selectedGroup?.id||item.groupId,pricing_settings};
 if(pd.sources?.minimum_price==='property'&&min) payload.min_price=min;
 if(pd.sources?.maximum_price==='property'&&max) payload.max_price=max;
 if(pd.sources?.rounding_increment==='property'&&pd.step) payload.rounding_increment=Number(String(pd.step).replace(/,/g,''));
...
const launchScrape=()=>{const start=new Date(scrapeForm.start),end=new Date(scrapeForm.end);if(!scrapeForm.groupId||Number.isNaN(start.valueOf())||Number.isNaN(end.valueOf())||end<start){setToast('Choose a pricing group and a valid date range.');return;}setBusy('comp');fetch('/api/competitor-scrapes',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken()},body:JSON.stringify({pricing_group_id:Number(scrapeForm.groupId),start_date:scrapeForm.start,end_date:scrapeForm.end,force_refresh:scrapeForm.force,collection_mode:scrapeForm.mode})}).then(async r=>{const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.detail||'Competitor scrape failed');setScrapeOpen(false);setToast(`Competitor group run ${body.run_id} started.`);return refreshRunTimestamps().then(()=>{if(body.run_id)refreshRunWhenFinished(body.run_id);});}).catch(e=>setToast(e.message)).finally(()=>setBusy(null));};
...
<Select label="Pricing group" value={scrapeForm.groupId} onChange={e=>setScrapeForm(f=>({...f,groupId:e.target.value}))} options={pricingGroups.length?pricingGroups.map(g=>({label:g.name,value:String(g.id)})):[{label:'No pricing groups',value:''}]} />
...
<Select label="Collection mode" value={scrapeForm.mode} onChange={e=>setScrapeForm(f=>({...f,mode:e.target.value}))} options={[{label:'Precise — exact nearby prices',value:'precise'},{label:'Rough — lower-frequency fallback',value:'rough'}]} />
