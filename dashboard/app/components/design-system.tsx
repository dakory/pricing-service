...
export function Button({variant='primary',size='md',disabled=false,children,onClick,type='button',style:styleProp}) {
  ...
  const variants={primary:{background:'var(--action-primary)',color:'var(--text-inverse)'},accent:{background:'var(--action-accent)',color:'var(--action-accent-ink)',boxShadow:'var(--glow-accent)'},secondary:{background:'var(--action-secondary)',color:'var(--text-primary)',borderColor:'var(--border-default)'},ghost:{background:'transparent',color:'var(--text-primary)'}};
  const [hover,setHover]=React.useState(false);
...
export function Input({label,type='text',placeholder,value,onChange,error,prefix,numeric=false,min,max,integer=false,loading=false}) {
  ...
  return <label style={{display:'flex',flexDirection:'column',gap:6,fontFamily:'var(--font-sans)'}}>
    ...
    <div style={{position:'relative',display:'flex',alignItems:'center',gap:8,height:38,boxSizing:'border-box',border:`1px solid ${border}`,borderRadius:'var(--radius-full)',padding:'0 20px',background:'var(--surface-card)',transition:'border-color var(--duration-fast) var(--ease-standard)'}}>
      {prefix&&<span style={{color:'var(--text-muted)',fontSize:14}}>{prefix}</span>}
      {loading?<span className="field-value-skeleton" aria-label="Loading value"/>:<input type={type} inputMode={numeric?'decimal':undefined} placeholder={placeholder} value={value} onChange={handleChange} onFocus={()=>setFocus(true)} onBlur={handleBlur} style={{border:'none',outline:'none',fontSize:14,fontFamily:'var(--font-sans)',color:'var(--text-primary)',width:'100%',background:'transparent'}}/>}
    </div>
...
export function InputWithSelectField({label,options=[],value,onChange,placeholder='Enter a value',currency=false,loading=false}) {
