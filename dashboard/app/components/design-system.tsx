...
export function Input({label,type='text',placeholder,value,onChange,error,prefix}) {
  ...
  const border = error ? 'var(--status-danger)' : focus ? 'var(--focus-ring)' : 'var(--border-default)';
  return <label style={{display:'flex',flexDirection:'column',gap:6,fontFamily:'var(--font-sans)'}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:8,height:38,boxSizing:'border-box',border:`1px solid ${border}`,borderRadius:'var(--radius-full)',padding:'0 20px',background:'var(--surface-card)',transition:'border-color var(--duration-fast) var(--ease-standard)'}}>
      ...
      <input type={type} placeholder={placeholder} value={value} onChange={onChange} onFocus={()=>setFocus(true)} onBlur={()=>setFocus(false)} style={{border:'none',outline:'none',fontSize:14,fontFamily:'var(--font-sans)',color:'var(--text-primary)',width:'100%',background:'transparent'}}/>
    </div>
