...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  return <div style={{ display: "flex", minHeight: "100vh", fontFamily: "var(--font-sans)", background: "var(--surface-page)", position: "relative", overflow: "hidden" }}>
    <div style={{display:collapsed?'contents':'block'}}>{!collapsed&&sidebarContent}{collapsed&&<><div onMouseEnter={() => setPeek(true)} style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: 14, zIndex: 39 }} />{!peek && <div style={{ position: "fixed", left: 24, top: 20, zIndex: 41 }}><IconButton label="Open sidebar" onClick={() => setCollapsed(false)} size={32} icon={<img src="https://unpkg.com/lucide-static@latest/icons/chevrons-right.svg" style={{ width: 15, height: 15 }} alt="" />} /></div>}<div style={{ position: "fixed", left: 0, top: 0, height: "100vh", zIndex: 40, transform: `translateX(${peek ? "0" : "-100%"})`, transition: "transform var(--duration-standard) var(--ease-standard)", boxShadow: peek ? "var(--shadow-lg)" : "none" }}>{sidebarContent}</div></>}</div>
    <div style={{ flex: 1, position: "relative", zIndex: 1, display: "flex", flexDirection: "column", minHeight: "100vh", minWidth: 0, paddingTop: 40, boxSizing: "border-box" }}><div style={{ padding: "24px 0 0", flex: 1, display: "flex", minHeight: 0, minWidth: 0 }}>{children}</div></div>
  </div>;
}
