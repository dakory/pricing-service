const {Input,IconButton,Select,Button,Toast,Switch,InputWithSelectField}=window.NicerHomesDesignSystem_ea7f10;
...
setBusy(key);
setTimeout(()=>{setBusy(null);setToast(label);if(key==='fetch')setLastSync(s=>({...s,fetch:'Just now'}));if(key==='comp')setLastSync(s=>({...s,comp:'Just now'}));if(key==='generate')setLastSync(s=>({...s,generate:'Just now'}));if(key==='apply')setLastSync(s=>({...s,apply:'Just now'}));setTimeout(()=>setToast(null),2600);},900);
};
